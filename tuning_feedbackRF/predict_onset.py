...
# present stimulus for 0.5s with 1s of grey screen in between as per Keller et al. 2020
BLANK_SIZE, PATTERN_SIZE = 15, 15
BLOCK_SIZE = BLANK_SIZE + PATTERN_SIZE + BLANK_SIZE
MAX_FRAME = data.MAX_FRAME
PATTERNS_PER_TRIAL = 5
...
def create_stimulus(args) -> tuple[torch.Tensor, np.ndarray]:
    ...
    num_patterns = len(parameters) * args.num_repeats
    assert (
    ...
    parameters = repeat(
        parameters, "block param -> (repeat block) param", repeat=args.num_repeats
    )
    ...
    for i in tqdm(range(parameters.shape[0]), desc="generate stimuli"):
...
def main(args):
    ...
    save_dir = args.output_dir / "feedbackRF_onset"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stimulus_size", type=int, default=20)
    parser.add_argument(
        "--num_repeats",
        type=int,
        default=500,
        help="Number of repetitions for each unique stimulus",
    )
