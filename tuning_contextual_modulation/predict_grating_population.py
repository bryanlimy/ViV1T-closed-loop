...
def inference(
    ...
    responses = torch.zeros((num_neurons, num_trials, num_frames), dtype=torch.float32)
    ...
    for i, batch in enumerate(tqdm(ds, desc=desc)):
        ...
        if animate and i in make_plot:
            plot.animate_stimulus(
                ...
                response=torch.mean(response[0, :, -num_frames:], dim=1).numpy(),
                filename=save_dir
                / "figures"
                / f"mouse{mouse_id}"
                / f"trial{i:04d}.mp4",
        ...
        response = rearrange(
            response[:, :, -num_frames:], "trial neuron frame -> neuron trial frame"
        )
...
def process_mouse(
    ...
    ds = DataLoader(
        dataset=GratingDataset(
            mouse_id=mouse_id,
            RF_center=stimulus.load_population_RF_center(
...
def main(args):
    ...
    save_dir = args.output_dir / "contextual_modulation_population"
