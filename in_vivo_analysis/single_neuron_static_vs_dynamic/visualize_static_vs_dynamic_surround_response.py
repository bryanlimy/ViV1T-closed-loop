...
def add_p_value(
    ...
    plot.add_p_value(
        ...
        tick_linewidth=1,
        text_pad=(0.6 * unit) if p_value < 0.05 else 0.5 * unit,
    )
    return p_value
...
def plot_center_surround_response(
    ...
    print(
        f"\tNumber of neurons: {len(static_center_responses)}\n"
    ...
    responses = [nsns_exciting, nsnd_exciting, nsgs_exciting, nsgd_exciting]
    ...
    print(f"\tnatural image vs natural video exciting p-value: {p_value:.03e}")
    p_value = add_p_value(
        ax=ax,
    ...
    print(f"\tgenerated image vs generated video p-value: {p_value:.03e}")
    p_value = add_p_value(
        ax=ax,
        response1=nsns_exciting,
        response2=nsgs_exciting,
        position1=x_ticks[0],
        position2=x_ticks[2],
        max_value=1.14 * max_value,
        unit=unit,
        num_compare=num_compare,
        alternative="less",
    )
    print(f"\tnatural image vs generated image p-value: {p_value:.03e}")
    p_value = add_p_value(
        ax=ax,
        ...
        position1=x_ticks[1],
        position2=x_ticks[3],
        max_value=1.25 * max_value,
        unit=unit,
        num_compare=num_compare,
        alternative="less",
    )
    print(f"\tnatural video vs generated video p-value: {p_value:.03e}")
    ...
    for n in range(len(static_center_responses)):
        for indexes in [(0, 1), (2, 3)]:
            ax.plot(
                [x_ticks[indexes[0]], x_ticks[indexes[1]]],
                np.clip(
                    [responses[i][n] for i in indexes],
                    a_min=0,
                    a_max=max_value,
                ),
                **line_kw,
    ...
    xlim = [x_ticks[0] - 0.35, x_ticks[-1] + 0.3]
...
def compute_gain(df: pd.DataFrame):
    natural_center = df[df.stimulus_type == "center"].response.values
    ...
    # normalize response by response to natural center
    natural_static /= natural_center
    natural_dynamic /= natural_center
    generated_static /= natural_center
    generated_dynamic /= natural_center
...
def plot_response_gain(
    ...
    print(
        ...
        f"\tnatural image mean: {np.mean(predicted_gain[0]):.0f} median: {np.median(predicted_gain[0]):.0f}\n"
        f"\tnatural video mean: {np.mean(predicted_gain[1]):.0f} median: {np.median(predicted_gain[1]):.0f}\n"
        f"\tgenerated image mean: {np.mean(predicted_gain[2]):.0f} median: {np.median(predicted_gain[2]):.0f}\n"
        f"\tgenerated video mean: {np.mean(predicted_gain[3]):.0f} median: {np.median(predicted_gain[3]):.0f}\n"
    )

    print(
        f"In vivo response gain over center\n"
        f"\tnatural image mean: {np.mean(recorded_gain[0]):.0f} median: {np.median(recorded_gain[0]):.0f}\n"
        f"\tnatural video mean: {np.mean(recorded_gain[1]):.0f} median: {np.median(recorded_gain[1]):.0f}\n"
        f"\tgenerated image mean: {np.mean(recorded_gain[2]):.0f} median: {np.median(recorded_gain[2]):.0f}\n"
        f"\tgenerated video mean: {np.mean(recorded_gain[3]):.0f} median: {np.median(recorded_gain[3]):.0f}\n"
    )
