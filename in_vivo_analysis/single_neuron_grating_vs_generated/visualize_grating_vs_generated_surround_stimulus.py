...
def load_grating_parquet(
    filename: Path,
    response_type: str,
    surround_direction: int | None = None,
) -> (np.ndarray, np.ndarray):
    df = pd.read_parquet(filename)
    if surround_direction is not None:
        df = df[df.surround_direction == surround_direction]
    else:
        center_direction = (
        ...
        df = df[
            (df.center_direction == center_direction) & (df.surround_direction != -1)
        ]
    ...
    response = df.raw_response[5:-5]
    stimulus = rearrange(df.stimulus, "(c t h w) -> c t h w", c=1, h=HEIGHT, w=WIDTH)
...
def load_predicted_responses(
    ...
    # load response to most-exciting grating center
    ...
    # load response to most-exciting grating surround
    responses[1], visual_stimuli[1] = load_grating_parquet(
        filename=save_dir
        / "gratings"
        / "center_surround"
        / f"mouse{mouse_id}_neuron{neuron:04d}.parquet",
    ...
    # load response to most-inhibiting grating surround
    responses[2], visual_stimuli[2] = load_grating_parquet(
        filename=save_dir
        / "gratings"
        / "center_surround"
        / f"mouse{mouse_id}_neuron{neuron:04d}.parquet",
    ...
    responses[3], visual_stimuli[3] = load_natural_parquet(
        filename=save_dir
        / "gratings"
    ...
    responses[4], visual_stimuli[4] = load_natural_parquet(
        filename=save_dir
        / "gratings"
        / "grating_center_natural_surround"
        / "dynamic_center_dynamic_surround"
...
def plot_neuron(
    ...
    lefts = np.linspace(0.04, 0.815, num=4)
    ...
    # most-exciting grating surround
    gs = GridSpec(
        nrows=1,
        ncols=1,
        left=lefts[1],
...
def process_mouse(
    ...
    for neuron in tqdm(neurons, desc=f"mouse {mouse_id}"):
        ...
        plot_neuron(
            ...
            output_dir=output_dir,
            filename=plot_dir
            / f"mouse{mouse_id}_neuron{neuron:04d}_grating_vs_generated_stimulus.{FORMAT}",
        )
