...
def inference(
    ...
    for i, batch in enumerate(tqdm(ds, desc=f"Mouse {mouse_id}")):
        ...
        if animate and i in make_plot:  # plot 10% of the trials
            animate_stimulus(
                video=batch["raw_video"][0, :, start:end, :, :],
                response=torch.mean(response[0, :, start:end], dim=0),
                filename=save_dir
                / "figures"
                / f"mouse{mouse_id}"
                / f"mouse{mouse_id}_center{batch['center_direction'][0]}_surround{batch['surround_direction'][0]}.mp4",
        ...
        center_directions[index : index + batch_size] = batch["center_direction"]
        surround_directions[index : index + batch_size] = batch["surround_direction"]
    ...
    df = pd.DataFrame(
        {
            ...
            "center_direction": center_directions.tolist(),
            "surround_direction": surround_directions.tolist(),
            "response": population_sum_response,
            "raw_response": responses.tolist(),
            "stimulus_type": ["dynamic"] * len(responses),
            "stimulus": rearrange(visual_stimuli, "b c t h w -> b (c t h w)").tolist(),
        }
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stimulus_size", type=int, default=20, help="stimulus size to use."
    )
    parser.add_argument(
        "--outer_stimulus_size",
        type=int,
