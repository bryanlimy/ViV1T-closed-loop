...
def inference(
    ...
    responses = torch.zeros(num_samples, dtype=torch.int32)
    index = 0
    for i, batch in enumerate(tqdm(ds, desc=f"Mouse {mouse_id}")):
        ...
        # sum response within the stimulus presentation window, average over
        # population, then round response to nearest integer to save space
        response = response[:, :, -(BLANK_SIZE + PATTERN_SIZE) : -BLANK_SIZE]
        response = torch.mean(torch.sum(response, dim=-1), dim=-1)
        response = torch.round(response, decimals=0)
...
def record_response(
    ...
    responses_ = torch.zeros(num_directions, dtype=torch.int32)
    ...
    max_response, preferred_direction = torch.max(responses_, dim=0)
    preferred_direction = unique_directions[preferred_direction]
    most_exciting = pd.DataFrame(
        {
            "direction": [preferred_direction.item()],
            "response": [max_response.item()],
        }
    )
    most_exciting.insert(loc=1, column="response_type", value="most_exciting")
    # get most-inhibiting response, trial and frame
    min_response, unpreferred_direction = torch.min(responses_, dim=0)
    unpreferred_direction = unique_directions[unpreferred_direction]
    most_inhibiting = pd.DataFrame(
        {
            "direction": [unpreferred_direction.item()],
            "response": [min_response.item()],
        }
    )
