...
PLOT_DIR = Path("figures") / "grating_vs_generated_stimulus"
...
def load_grating_results(
    filename: Path, mouse_id: str, neurons: list[int] | np.ndarray
):
...
def plot_stimulus(
    ...
    if title is not None:
        ...
        ax.text(
            ...
            y=-0.2 * h if "center" in title.lower() else -0.16 * h,
            s=title,
...
def plot_neuron(
    grating_result: dict[str, dict[str, np.ndarray]],
    natural_result: dict[str, dict[str, np.ndarray]],
    generated_result: dict[str, dict[str, np.ndarray]],
    mouse_id: str,
    neuron: int,
    filename: Path,
) -> None:
    grating_responses = np.array([v["response"] for k, v in grating_result.items()])
    natural_responses = np.array([v["response"] for k, v in natural_result.items()])
    generated_responses = np.array([v["response"] for k, v in generated_result.items()])

    # normalize responses to static and dynamic stimuli by the max response to
    # static center stimulus
    max_center_response = np.max(grating_responses[0])
    grating_responses /= max_center_response
    natural_responses /= max_center_response
    generated_responses /= max_center_response

    x = np.arange(len(grating_result["center"]["response"]))
    ...
    plot_stimulus(
        ax=axes2[0],
        stimulus=grating_result["most_exciting"]["stimulus"][0, :],
        figure=subfigure,
    ...
    plot_response(
        ax=axes2[1],
        x=x,
        response=grating_responses[1],
        presentation_mask=presentation_mask,
        x_ticks=x_ticks,
        x_tick_labels=[],
        x_label=None,
        y_label=None,
        color=GRATING_SURROUND_COLOR,
    )
    ...
    plot_stimulus(
        ax=axes3[0],
        stimulus=grating_result["most_inhibiting"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
    ...
    plot_stimulus(
        ax=axes4[0],
        stimulus=natural_result["center"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
        title="Most-exciting\ngrating centre",
        y_label="",
    ...
    plot_stimulus(
        ax=axes5[0],
        stimulus=natural_result["most_exciting"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
        title=None,
        y_label=None,
    ...
    plot_response(
        ax=axes5[1],
        x=x,
        response=natural_responses[1],
        presentation_mask=presentation_mask,
        x_ticks=x_ticks,
        x_tick_labels=[],
        x_label=None,
        y_label=None,
        color=DYNAMIC_NATURAL_COLOR,
    )
    ...
    plot_stimulus(
        ax=axes6[0],
        stimulus=natural_result["most_inhibiting"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
        title=None,
        y_label=None,
    ...
    plot_stimulus(
        ax=axes8[0],
        stimulus=generated_result["most_exciting"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
        title=None,
        y_label=None,
    ...
    plot_stimulus(
        ax=axes9[0],
        stimulus=generated_result["most_inhibiting"]["stimulus"][0, :],
        figure=subfigure,
        imshow_kws=imshow_kws,
        title=None,
        y_label=None,
    ...
    plot_response(
        ax=axes9[1],
        x=x,
        response=generated_responses[2],
        presentation_mask=presentation_mask,
        x_ticks=x_ticks,
        x_tick_labels=[],
        x_label=None,
...
def process_model(model_name: str, output_dir: Path):
    ...
    for mouse_id in data.SENSORIUM_OLD:
        ...
        gratings_results = load_grating_results(
            filename=save_dir / "grating_center_grating_surround.pkl",
            mouse_id=mouse_id,
            neurons=neurons,
        )
        natural_results = load_natural_results(
            filename=save_dir / "grating_center_natural_surround.pkl",
            mouse_id=mouse_id,
            neurons=neurons,
        )
        generated_results = load_generated_results(
            save_dir=save_dir
            / "generated"
            / "center_surround"
            / "grating_center"
        ...
        #     plot_neuron(
        #         grating_result=gratings_results[neuron],
        #         natural_result=natural_results[neuron],
        #         generated_result=generated_results[neuron],
        #         mouse_id=mouse_id,
        #         neuron=neuron,
        #         filename=PLOT_DIR
        #         / f"mouse{mouse_id}"
        #         / f"grating_vs_generated_stimulus_mouse{mouse_id}_neuron{neuron:04d}.{FORMAT}",
        #     )
        neuron = 4264
        plot_neuron(
            grating_result=gratings_results[neuron],
