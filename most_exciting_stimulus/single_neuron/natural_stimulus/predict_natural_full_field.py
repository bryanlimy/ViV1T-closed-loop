...
def inference(
    ...
    with logging_redirect_tqdm(loggers=[logger]):
        for i, batch in enumerate(
            ...
            response = response[:, neuron, -(BLANK_SIZE + PATTERN_SIZE) : -BLANK_SIZE]
            ...
            index += batch_size
    return responses, trial_ids, frames
...
def process_mouse(
    ...
    for neuron in reliable_neurons:
        ...
        responses, trial_ids, frames = inference(
            model=model,
            mouse_id=mouse_id,
            neuron=neuron,
            ds=ds,
        ...
        result = pd.DataFrame(
            {
                ...
                "stim_type": ["static" if args.static else "dynamic"],
            }
        )
