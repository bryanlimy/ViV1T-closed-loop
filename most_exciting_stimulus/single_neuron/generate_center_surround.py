...
from viv1t.most_exciting_stimulus import parameters
...
def get_most_exciting_natural_center(
    args, mouse_id: str, neuron: int, device: torch.device
) -> torch.Tensor:
    static = args.static_center
    filename = (
        args.output_dir
    ...
    assert (
        filename.exists()
    ), f"Cannot find {filename}. Please run predict_natural_center.py first."
    # find the most exciting stimulus for neuron
    df = pd.read_parquet(filename)
    df = df[
        (df.mouse == mouse_id)
        & (df.neuron == neuron)
        & (df.stimulus_type == ("static" if static else "dynamic"))
    ]
    assert len(df) == 1
...
def get_most_exciting_grating_center(
    ...
    assert (
        args.static_center is False
    ), f"only dynamic stimulus is supported with grating center"
    filename = (
        args.output_dir
        / "most_exciting_stimulus"
        / "single_neuron"
...
def get_most_exciting_natural_surround(
    ...
    filename = args.output_dir / "most_exciting_stimulus" / "single_neuron"
...
def loss_function(
    ...
    match loss_mode:
        case "sum":
            loss = -torch.sum(response) if not inhibitory else torch.sum(response)
        case "max":
            loss = -torch.max(response) if not inhibitory else torch.max(response)
        case _:
...
def generate_stimulus(
    ...
    if center_video is None:
        if args.use_natural_center:
            center_video = get_most_exciting_natural_center(
                args, mouse_id=mouse_id, neuron=neuron, device=device
            )
            num_steps = 0
    ...
    match mode:
        ...
        case 1:
            logger.info(
                f"\nGenerate most-exciting {'static' if args.static_surround else 'dynamic'} surround stimulus"
            )
            save_dir = save_dir / "most_exciting"
            num_frames = 1 if args.static_surround else PATTERN_SIZE
        case 2:
            logger.info(
                f"\nGenerate most-inhibiting {'static' if args.static_surround else 'dynamic'} surround stimulus"
            )
    ...
    if args.natural_init:
        param = get_most_exciting_natural_surround(
            ...
            mouse_id=mouse_id,
            neuron=neuron,
            response_type=("most_exciting" if mode == 1 else "most_inhibiting"),
        )
    param, scale = parameters.get_param(
    ...
    start = -(BLANK_SIZE + PATTERN_SIZE + 20)
    stop = -(BLANK_SIZE - 20)
    ...
    if args.animate:
        animate_stimulus(
            ...
            response=val_result["response"][start:stop],
    ...
    if args.verbose > 1:
        logger.info(
            ...
            f"Sum: {val_result['sum_response']:.02f}"
        )
            ...
            for step in tqdm(range(1, num_steps + 1)):
                ...
                if step % (10 if num_steps <= 100 else 200) == 0 and args.verbose > 1:
                    logger.info(
                        ...
                        f"Loss: {train_result['loss']:.02f}, "
                        f'KL loss: {train_result["kl_loss"]:.02f}, '
                        f"Sum: {train_result['sum_response']:.02f}"
                    )
                if args.animate and step % 100 == 0:
                    animate_stimulus(
                        video=train_result["video"][:, start:stop],
                        response=train_result["response"][start:stop],
    ...
    animate_stimulus(
        ...
        response=final_result["response"][start:stop],
    ...
    if args.verbose:
        logger.info(
            ...
            f"KL Loss: {final_result['kl_loss']:.02f}, "
            f"Sum: {final_result['sum_response']:.02f}"
        )
    # return center video
...
def generate_center_surround_stimulus(
    ...
    save_dir = save_dir / f"mouse{mouse_id}" / f"neuron{neuron:04d}"
    if save_dir.is_dir():
        logger.info(
            f"\n\nGenerated center-surround stimulus for mouse {mouse_id} "
            f"neuron {neuron:04d} already exists in {save_dir}."
        )
        return
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    logger.info(
        ...
        f"for mouse {mouse_id} neuron {neuron}. (outer stimulus size "
    ...
    # generate the most exciting center or use most exciting natural frame/clip from Sensorium 2023
...
def main(args):
    assert not (
        args.use_natural_center and args.use_grating_center
    ), "You can only use either natural center or grating center."
    assert not (
        args.use_grating_center and args.static_center
    ), "Static grating center is not supported yet."
