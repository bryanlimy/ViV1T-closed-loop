...
PLOT_DIR = Path("figures") / "animate_stimuli" / "natural_center"

plot.set_font()


def get_natural_center(
    output_dir: Path,
    mouse_id: str,
    neuron: int,
    data: dict[str, list],
    ...
    data["stimulus_name"].append("natural_center")


def get_natural_image_surround(
    output_dir: Path,
    mouse_id: str,
    neuron: int,
    data: dict[str, list],
    ...
    data["response"].append(df.raw_response.astype(np.float32))
    data["stimulus_name"].append("natural_center_natural_image_surround")
...
def get_natural_video_surround(
    ...
    data["response"].append(df.raw_response.astype(np.float32))
    data["stimulus_name"].append("natural_center_natural_video_surround")


def get_generated_image_surround(
    output_dir: Path,
    experiment_name: str,
    mouse_id: str,
    neuron: int,
    ...
    data["stimulus_name"].append("natural_center_generated_image_surround")
...
def get_generated_video_surround(
    ...
    data["video"].append(ckpt["video"][:, start:end, :, :].detach().numpy())
    data["response"].append(ckpt["response"][start:end].detach().numpy())
    data["stimulus_name"].append("natural_center_generated_video_surround")
...
def main():
    df = pd.read_parquet("in_silico_single_neuron_natural_center.parquet")
    ...
    for mouse_id, output_dir_name, experiment_name in [
        ...
        for neuron in neurons:
            ...
            for i in tqdm(range(5), desc=f"mouse {mouse_id} neuron {neuron:03d}"):
                animate_stimulus(
                    video=data["video"][i],
